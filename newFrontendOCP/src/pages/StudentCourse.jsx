...
const StudentCourse = (props) => {
  ...
  const checkEnrollment = async()=>{
    try {
      const isStudent = await fetch(`http://localhost:4000/courseSystem/checkEnrollment/`, {
        method: 'POST',
        headers: {
  ...
  const enrollStudent =async()=>{
      try {
        const response = await fetch('http://localhost:4000/courseSystem/enroll', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
const handlStudentChat = async(e) =>{
  ...
  <div className="d-flex container justify-content-center negative-margin-top " >
    <div className="col-md-4 d-flex flex-column align-items-center justify-content-center fullScreen">
      <div  style={{backgroundColor:'#f5f4f2',borderRadius:'25px'}} className="card d-flex mt-4 align-items-center justify-content-center">
        <div  className="card-body text-center" style={{margin:'25px',}}>
              <div className="d-flex flex-column">
                ...
                {enrolled ? (
                  <>
                  <button style={{border:'none',color:'whitesmoke'}} onClick={handleGetData} className="rounded  btnNavbarClr">
                    Get Course Data
                  </button>
                  <button style={{border:'none',color:'whitesmoke'}} onClick={handlStudentChat} className="rounded  btnNavbarClr mt-2">
                    Chat with the Teacher
                  </button>
                  </>
                ) : (
                  <button style={{border:'none',color:'whitesmoke'}} onClick={enrollStudent} className="mb-4 rounded googleKalnia  btnNavbarClr">
                    Enroll
                  </button>
                )}
              </div>
